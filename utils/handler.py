...
class DataHandler(DataBaseClient):
    ...
    def _formatting_tables(self, page_content):
    ...
    def __parse_and_formatting_date(self, table):
        exploit_date = re.findall(r'\d\d-\d\d-\d\d\d\d', str(table))
        if exploit_date:
            return self.__exploit_dict.update(date=exploit_date[-1])
        else:
            return self.__exploit_dict.update(date="Date not found")

    def __parse_and_formatting_description(self, table):
        ...
        if exploit_description:
            ...
            link = f"{URL}{link}"
            return self.__exploit_dict.update(description=description, description_link=link)
        else:
            return self.__exploit_dict.update(description="Description not found",
                                              description_link="Description link not found")

    def __parse_and_formatting_platform(self, table):
        ...
        if not exploit_platform:
            ...
            if not exploit_platform:
                return self.__exploit_dict.update(platform="Platform not found")
        exploit_platform = exploit_platform[0][:-4].split('>')[-1]
        return self.__exploit_dict.update(platform=exploit_platform)

    def __parse_and_formatting_risk(self, table):
        exploit_risk = re.findall(r'Security Risk .*<', str(table))[0][:-1]
        if exploit_risk:
            return self.__exploit_dict.update(risk=exploit_risk)
        else:
            return self.__exploit_dict.update(risk="Risk not found")

    def __parse_and_formatting_price(self, table):
        ...
        if exploit_btc:
            ...
            return self.__exploit_dict.update(price=exploit_btc)
        else:
            return self.__exploit_dict.update(price="Free")

    # todo sqlite
    def __write_in_json(self):
        with open('exploits.json', 'a') as file:
            json.dump(self.__exploit_dict, file, indent=4)
